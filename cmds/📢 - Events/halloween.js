...
module.exports.run = async (client, message, args, tools) => {
  ...
  let pages = [
    `${emotes.Info} - ${lang.Infomation_DESCRIPTION} - ${emotes.jason}`,
    `${emotes.Info} - ${lang.Infomation_DESCRIPTION} - ${emotes.halloitem}`,
  ];
  let title = [
    `${lang.Information} - ${lang.hallo_title}`,
    `${lang.Information} - ${lang.hallo_title}`,
  ];
  let url = [`${lang.hallo_link}`, `${lang.hallo_link}`];
  let image = [`${files.zombie}`, `${files.halloitems}`];
  let page = 1;
